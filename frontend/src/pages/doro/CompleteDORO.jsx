...
import { getCompleteDORO } from '@api/DORO';
...
function CompleteDORO() {

  const getUserToken = authHolder((state) => state.account_details)
  const token = jwtDecode(getUserToken)
  const queryClient = useQueryClient()
  const CompleteDOROListQuery = useQuery({
    queryKey: ['CompleteDOROList'],
    queryFn: async () => {
      const result = await getCompleteDORO(token.userKey)
      return result
    },
  ...
  return (
    <>
      <ViewData showModal={getModalStatusView} token={token} display={'Edit'} selected={'DORO'}
      ...
      <div className='px-2'>
        ...
        <DataTable width={'max-content'} height={'100vh'} column={'Normal'}
          rows={CompleteDOROListQuery.data?.filter((items) => {
