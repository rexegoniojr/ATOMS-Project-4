...
import { getPendingDORO } from '@api/DORO';
...
function PendingDORO() {

  const getUserToken = authHolder((state) => state.account_details)
  const token = jwtDecode(getUserToken)
  const queryClient = useQueryClient()
  const PendingDOROListQuery = useQuery({
    queryKey: ['PendingDOROList'],
    queryFn: async () => {
      const result = await getPendingDORO(token.userKey)
      return result
    },
    ...
    <>
      ...
      <div className='px-2'>
        ...
        <DataTable width={'max-content'} height={'100vh'} column={'Normal'}
          rows={PendingDOROListQuery.data?.filter((items) => {
