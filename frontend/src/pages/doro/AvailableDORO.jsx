...
import { getAvailableDORO } from '@api/DORO';
...
function AvailableDORO() {

  const getUserToken = authHolder((state) => state.account_details)
  const token = jwtDecode(getUserToken)
  const queryClient = useQueryClient()
  const AvailableDOROListQuery = useQuery({
    queryKey: ['AvailableDOROList'],
    queryFn: async () => {
      const result = await getAvailableDORO()
      return result
    },
    ...
    <>
      ...
      <div className='px-2'>
        ...
        <DataTable width={'max-content'} height={'100vh'} column={'Normal'}
          rows={AvailableDOROListQuery.data?.filter((items) => {
