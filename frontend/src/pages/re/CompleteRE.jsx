...
import { getCompleteRE } from '@api/RE';
...
function CompleteRE() {
  ...
  const CompleteREListQuery = useQuery({
    queryKey: ['CompleteREList'],
    queryFn: async () => {
      const result = await getCompleteRE(token.userKey)
      return result
    },
    ...
    <>
      ...
      <div className='px-2'>
        ...
        <DataTable width={'max-content'} height={'100vh'} column={'Normal'}
          rows={CompleteREListQuery.data?.filter((items) => {
