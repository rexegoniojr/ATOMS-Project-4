...
import { getPendingRE } from '@api/RE';
...
function PendingRE() {
  ...
  const PendingREListQuery = useQuery({
    queryKey: ['PendingREList'],
    queryFn: async () => {
      const result = await getPendingRE(token.userKey)
      return result
    },
    ...
    <>
      ...
      <div className='px-2'>
        ...
        <DataTable width={'max-content'} height={'100vh'} column={'Normal'}
          rows={PendingREListQuery.data?.filter((items) => {
