...
import { getAssignedPermitListToEmployee } from '@api/Permit';
...
function PermitAssigned() {

    const getUserToken = authHolder((state) => state.account_details)
    const token = jwtDecode(getUserToken)
    const queryClient = useQueryClient()
    const PreAlertWithPermitAssignedListQuery = useQuery({
        queryKey: ['PreAlertWithPermitAssigned'],
        queryFn: async () => {
            const result = await getAssignedPermitListToEmployee(token.userKey)
            return result
        },
    ...
    return (
        <>
            <ViewData showModal={getModalStatusView} token={token} display={'Edit'} selected={'Permit'}
            ...
            <div className='px-4'>
                ...
                <DataTable width={'max-content'} height={'100vh'} column={'Normal'}
                    rows={PreAlertWithPermitAssignedListQuery.data?.filter((items) => {
