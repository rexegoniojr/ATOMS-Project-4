...
import { getPendingPortal } from '@api/Portal';
...
function PendingPortal() {
    ...
    const PendingPortalListQuery = useQuery({
        queryKey: ['PendingPortalList'],
        queryFn: async () => {
            const result = await getPendingPortal(token.userKey)
            return result
        },
        ...
        <>
            ...
            <div className='px-2'>
                ...
                <DataTable width={'max-content'} height={'100vh'} column={'Normal'}
                    rows={PendingPortalListQuery.data?.filter((items) => {
