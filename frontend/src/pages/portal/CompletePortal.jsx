...
import { getCompletePortal } from '@api/Portal';
...
function CompletePortal() {
    ...
    const CompletePortalListQuery = useQuery({
        queryKey: ['CompletePortalList'],
        queryFn: async () => {
            const result = await getCompletePortal(token.userKey)
            return result
        },
        ...
        <>
            ...
            <div className='px-2'>
                ...
                <DataTable width={'max-content'} height={'100vh'} column={'Normal'}
                    rows={CompletePortalListQuery.data?.filter((items) => {
