...
import { getAvailablePortal } from '@api/Portal';
...
function AvailablePortal() {

    const getUserToken = authHolder((state) => state.account_details)
    const token = jwtDecode(getUserToken)
    const queryClient = useQueryClient()
    const AvailablePortalListQuery = useQuery({
        queryKey: ['AvailablePortalList'],
        queryFn: async () => {
            const result = await getAvailablePortal(token.userKey)
            return result
        },
    ...
    return (
        <>
            <ViewData showModal={getModalStatusView} token={token} display={'Edit'} selected={'Portal'}
            ...
            <div className='px-2'>
                ...
                <DataTable width={'max-content'} height={'100vh'} column={'Normal'}
                    rows={AvailablePortalListQuery.data?.filter((items) => {
...
export default AvailablePortal