...
import { getPeerCheckerAssignedList } from '@api/PeerChecker';
...
function PeerChecker() {

    const getUserToken = authHolder((state) => state.account_details)
    const token = jwtDecode(getUserToken)
    const queryClient = useQueryClient()
    const AssignedPeerCheckerListQuery = useQuery({
        queryKey: ['AssignedPeerCheckerListToUser'],
        queryFn: async () => {
            const result = await getPeerCheckerAssignedList(token.userKey)
            return result
        },
    ...
    return (
        <>
            <ViewData showModal={getModalStatusView} token={token} display={'Edit'} selected={'Peer Checker'}
            ...
            <div className='px-4'>
                ...
                <DataTable width={'max-content'} height={'100vh'} column={'Normal'}
                    rows={AssignedPeerCheckerListQuery.data?.filter((items) => {
