...
import { getPreAlertWithPermit } from '@api/Permit';
...
function AssignPermit() {
    ...
    const PreAlertWithPermitListQuery = useQuery({
        queryKey: ['PreAlertWithPermitList'],
        queryFn: async () => {
            const result = await getPreAlertWithPermit()
            return result
        },
    ...
    return (
        <>
            <ViewData showModal={getModalStatusView} token={token} display={'Assign'} selected={'Permit'}
            ...
            <div className='px-4'>
                ...
                <DataTable width={'max-content'} height={'100vh'} column={'Normal'}
                    rows={PreAlertWithPermitListQuery.data?.filter((items) => {
