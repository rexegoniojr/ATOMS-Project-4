...
import { getLodgeList } from '@api/Lodge';
...
function Lodgement() {
    ...
    const LodgeListQuery = useQuery({
        queryKey: ['LodgeList'],
        queryFn: async () => {
            const result = await getLodgeList(token.userKey)
            return result
        },
    ...
    return (
        <>
            <ViewData showModal={getModalStatusView} token={token} display={'Edit'} selected={'Lodgement'}
            ...
            <div className='px-4'>
                ...
                <DataTable width={'max-content'} height={'100vh'} column={'Normal'}
                    rows={LodgeListQuery.data?.filter((items) => {
