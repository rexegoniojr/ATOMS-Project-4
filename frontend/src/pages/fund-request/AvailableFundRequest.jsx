...
import { Button, Tooltip, Input, Tag, Tabs } from 'antd';
import { SearchOutlined } from '@ant-design/icons';
import { useQuery, useQueryClient } from '@tanstack/react-query';
import { getAvailableFundRequestList } from '@api/FundRequest';
...
function AvailableFundRequest() {

  const getUserToken = authHolder((state) => state.account_details)
  const token = jwtDecode(getUserToken)
  const queryClient = useQueryClient()
  const AvailableFundRequestListQuery = useQuery({
    queryKey: ['AvailableFundRequestList'],
    queryFn: async () => {
      const result = await getAvailableFundRequestList(token.userKey)
      const container = [...new Map(result?.map(x => [x.refNum, x])).values()]
      return container
    },
  ...
  return (
    <>
      <ViewData showModal={getModalStatusView} token={token} display={'Edit'} selected={'Fund Request'}
      ...
      <div className='px-2'>
        ...
        <DataTable width={'max-content'} height={'100vh'} column={'Normal'}
          rows={AvailableFundRequestListQuery.data?.filter((items) => {
...
export default AvailableFundRequest