...
import { getCDTAssignedList } from '@api/CDT';
...
function CDT() {
    ...
    const ForCDTListQuery = useQuery({
        queryKey: ['AssignedCDTListToUser'],
        queryFn: async () => {
            const result = await getCDTAssignedList(token.userKey)
    ...
    return (
        <>
            <ViewData showModal={getModalStatusView} token={token} display={'Edit'} selected={'CDT'}
            ...
            <div className='px-4'>
                ...
                <DataTable width={'max-content'} height={'100vh'} column={'Normal'}
                    rows={ForCDTListQuery.data?.filter((items) => {
